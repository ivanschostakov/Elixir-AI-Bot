...
async def handle_user_start(message: Message, state: FSMContext, professor_bot, professor_client):
    ...
    response = await safe_ai_response(message, send_message_v2_from_telegram(message=message, professor_client=professor_client, user_id=user_id, conversation_id=user.conversation_id, input_text_override=f"Я написал первое сообщение или возобновил наш диалог. Начни/возобнови диалог. Мое имя в Telegram — {message.from_user.full_name}."))
...
async def handle_user_registration(message: Message, state: FSMContext, professor_bot, professor_client):
    ...
    await state.clear()
    conversation_id = await professor_bot.create_user(message.from_user.id, normalize_phone(phone), message.from_user.first_name, message.from_user.last_name)
    assistant_id = _resolve_assistant_id(message)
    response = await safe_ai_response(message, send_message_v2_from_telegram(message=message, professor_client=professor_client, user_id=message.from_user.id, conversation_id=conversation_id, input_text_override=f"ОБРАЩАЙСЯ ТОЛЬКО НА ВЫ, Я написал первое сообщение. Мое имя в Telegram — {message.from_user.full_name}."))
    if response is None: return None
    schedule_webapp_call(
        safe_webapp_call(
            webapp_client.write_usage(
...
async def handle_new_chat(message: Message, state: FSMContext, professor_client):
    result = await CHAT_NOT_BANNED_FILTER(message)
    if not result: return await message.answer(user_texts.banned_in_channel)
    conversation_id = await professor_client.create_conversation(user_id=message.from_user.id)
...
@professor_user_router.message(lambda message: not message.media_group_id and (message.photo or message.video or message.video_note or message.document or message.voice))
...
@professor_user_router.message(lambda message: message.text and message.text.strip())
...
async def handle_text_message(message: Message, state: FSMContext, professor_bot, professor_client):
    ...
    response = await safe_ai_response(message, send_message_v2_from_telegram(message=message, professor_client=professor_client, user_id=user_id, conversation_id=user.conversation_id))
    if response is None: return None
    schedule_webapp_call(
        safe_webapp_call(
            webapp_client.write_usage(
